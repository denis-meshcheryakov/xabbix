...
from forms import GetCommand
from send_command_to_router import send_command
...
def send_received_cmd(device):
    ...
    if command_form.validate_on_submit():
        ...
        output = send_command(device=device, command=get_command, command_type=command_type)
        output = output.split('\n')
        for i in output:
            flash(i)
    return command_form
